...
const typePetCollection = db.collection(db,"TypePets");
...
router.get("/", async (req, res) => {
    try {
      ...
      const typePetData = typePetRef.docs.map((doc) => {
        return { id_TypePet: doc.id, ...doc.data() };
      });
...
router.post('/', async (req, res) => {
    try {
      ...
      const fileRef = storageRef.child(`images/TypePet/${Date.now()}_${req.body.nameType}`);
