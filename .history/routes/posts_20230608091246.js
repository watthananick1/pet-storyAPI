...
// Get posts by user's first name
router.get("/user/:firstName", async (req, res) => {
  try {
    ...
    posts[index].firstName = user.firstName; // Add firstName to comment
    posts[index].lastName = user.lastName; // Add lastName to comment
    posts[index].profilePicture = user.profilePicture; // Add profilePicture to comment
