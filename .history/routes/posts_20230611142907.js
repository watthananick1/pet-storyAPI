...
router.get("/user/:firstName/:sort", validateToken, async (req, res) => {
  ...
  try {
    const isUserDoc = await usersCollection.where("member_id", "==", post.member_id)
    .get();
...
router.get("/:id/:sort", validateToken, async (req, res) => {
  try {
    ...
    const sortParam = req.params.sort;
    console.log("memberId", req.user.userId);
    const isUserDoc = await usersCollection.where("member_id", "==", req.user.userId)
    .get();
    const isUser = isUserDoc.data();
    if (isUser) {
